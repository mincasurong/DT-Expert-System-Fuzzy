...
global Point; global Enter; Enter = input('Enter [ Default(0), Insert(1) ]: ');
if Enter == 0, Point =  [75 13 85 -5 -3]; % 40~140, 5~30, 60~160, -20~20, -20~20
else , Point = input('Enter the parameters : \n [HR (40~140) RR (5~30) BP (60~160) ST_toq (-20~20) SW_toq (-20~20)]\n ')
end
...
if max(mu_mandani) < 0.6, mu_mandani=zeros(1,4); mu_mandani(1)=1; mu_mandani
else, mu_mandani, end
...
for k=1:4, 
    if mu_mandani(k) == max(mu_mandani)
       if k==1,     bar(k,mu_mandani(k),'r','barwidth',0.2); legend('Reject','Decision : Easy'); 
       elseif k==2, bar(k,mu_mandani(k),'r','barwidth',0.2); legend('Reject','Decision : Maintain')
       elseif k==3, bar(k,mu_mandani(k),'r','barwidth',0.2); legend('Reject','Decision : Hard')
       elseif k==4, bar(k,mu_mandani(k),'r','barwidth',0.2); legend('Reject','Decision : Too Hard')    
       end       
    end
end
